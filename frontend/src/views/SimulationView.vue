  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('Simulation state is running, stopping...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
    // Environment status check failure doesn't affect subsequent workflow
...
const forceStopSimulation = async () => {
  try {
    ...
    addLog(`Exception checking simulation status: ${err.message}`)
  }
}
